...
export default function AdminProjectCard({ 
  ...
  try {
    console.log('=== AdminProjectCard RENDER ===', project.id, project.title)
    const { t } = useLanguage()
    ...
    const getSafeThumbnailUrl = () => {
      ...
      if (!project.thumbnail_url || project.thumbnail_url === 'undefined') {
        console.warn(`AdminProject ${project.id} has invalid thumbnail_url:`, project.thumbnail_url)
      }
      ...
      console.log(`Using fallback for admin project ${project.id}:`, fallbackUrl)
      return fallbackUrl
    }

    // Get safe images array
    const getSafeImages = () => {
      console.log(`=== AdminProject ${project.id} Image Debug ===`)
      ...
      console.log(`Final filtered images for admin project ${project.id}:`, filteredImages)
  ...
  console.log(`AdminProject ${project.id}: images=${project.images?.length}, safe=${safeImages.length}, multiple=${hasMultipleImages}`)
  
  // Debug the layout decision
  console.log(`AdminProject ${project.id} layout decision:`)
  ...
  return (
    ...
    >
      {/* Main Content */}
      <div className={`${!hasMultipleImages ? '' : 'grid grid-cols-1 lg:grid-cols-3 gap-0'}`}>
        ...
        <div className={`${!hasMultipleImages ? '' : 'lg:col-span-2'} relative`}>
          <div className={`${!hasMultipleImages ? 'aspect-[16/9]' : 'aspect-[4/3] lg:aspect-[16/9]'} overflow-hidden w-full h-full rounded-lg`}>
            <img
    ...
    console.error('AdminProjectCard ERROR:', error)
    console.error('Project data:', project)
    return (
      <div className="bg-red-900 border border-red-500 rounded-lg p-4 text-white">
        <h3 className="text-lg font-bold mb-2">Error loading admin project</h3>
